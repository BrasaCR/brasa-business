...
Operator onboarding and access changes use the local, Cloudflare-authenticated staging command described in `docs/MARKETPLACE_OPERATORS.md`; no internet-facing administrator API exists. Creation, invitation, role change, suspension, and resumption write a separate operator audit trail. Suspension blocks the Business authorization boundary before Identity credentials are revoked, while resumption requires a fresh one-time invitation.

Direct administrator grants are blocked. An active verifier must initiate an expiring elevation request and a different active verifier or administrator must approve it. The staging readiness command also checks for an approved 365–2555 day audit-retention period and two distinct active emergency owners, but remains fail-closed for production while operator authentication is based on one-time invitations rather than phishing-resistant credentials.
