...
The staging review desk is available at `/operator/marketplace`. Its credential is held only in page memory and cleared by the Lock action or when the tab closes. It provides provider queues, report review, draft creation, and state actions. The first-provider checklist is enforced in both the interface and the API: appropriate source authority, matched provenance, planned expiry, documented consent or legal basis, and an assigned responsible human reviewer must all be confirmed before a pending record can be created. Verification remains a separate action.
