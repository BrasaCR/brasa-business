...
export async function createOperatorProvider(request, env) {
  ...
  const allowed = new Set(['id','name','category','description','capabilities','countryCode','region','websiteUrl','provenanceUrl','sourceLabel','reason','approvals']);
  const approvalKeys = ['authority','provenance','expiry','consentLegalBasis','responsibleReviewer'];
  if (Object.keys(input).some((key) => !allowed.has(key)) || !input.approvals || Object.keys(input.approvals).length !== approvalKeys.length || !approvalKeys.every((key) => input.approvals[key] === true) || !validId(input.id) || typeof input.name !== 'string' || !input.name.trim() || input.name.length > 160 || !/^[a-z0-9-]{1,80}$/.test(input.category || '') || typeof input.description !== 'string' || !input.description.trim() || input.description.length > 1000 || !Array.isArray(input.capabilities) || input.capabilities.length > 20 || input.capabilities.some((item) => typeof item !== 'string' || !/^[a-z0-9-]{1,80}$/.test(item)) || !/^[A-Z]{2}$/.test(input.countryCode || '') || (input.region != null && (typeof input.region !== 'string' || input.region.length > 160)) || !validUrl(input.websiteUrl) || !validUrl(input.provenanceUrl) || typeof input.sourceLabel !== 'string' || !input.sourceLabel.trim() || input.sourceLabel.length > 160 || typeof input.reason !== 'string' || input.reason.trim().length < 3 || input.reason.length > 500) return json({ error: 'invalid_provider_record' }, 400);
